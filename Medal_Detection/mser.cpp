...
cx(0), cy(0), e0(0), e1(0), e0sq(0), e1sq(0), v0x(0), v0y(0), v1x(0), v1y(0), n0(0), n1(0)
{
	fill_n(moments_, 5, 0.0);
}

void MSER::Region::estimate()
{
	// Centroid (mean)
	cx = moments_[0] / area_;
	cy = moments_[1] / area_;
	...
	const double a = moments_[2] / area_ - cx * cx;
	const double b = moments_[3] / area_ - cx * cy;
	const double c = moments_[4] / area_ - cy * cy;

	// Eigenvalues of the covariance matrix
	const double d = a + c;
	const double e = a - c;
	const double f = sqrt(4.0 * b * b + e * e);
	e0 = (d + f) / 2.0; // First eigenvalue
	e1 = (d - f) / 2.0; // Second eigenvalue
	...
	if (b) {
		...
		n0 = e0sq / sqrt(v0x * v0x + v0y * v0y);
		v0x *= n0;
		v0y *= n0;

		n1 = e1sq / sqrt(v1x * v1x + v1y * v1y);
		v1x *= n1;
		v1y *= n1;
	}
}
...
{
	if (stable_) {
		const int minParentArea = int(area_ / (1.0 - minDiversity) + 0.5);
		...
		if (stable_) {
			const int maxChildArea = int(area_ * (1.0 - minDiversity) + 0.5);
...
step_3:
	...
	for (;;) {
		...
		if (priority == 256) {
			regionStack.back()->detect(delta_, int(minArea_ * width * height + 0.5),
									   int(maxArea_ * width * height), maxVariation_, minDiversity_,
									   regions);
			poolIndex_ = 0;
			return;
		}
