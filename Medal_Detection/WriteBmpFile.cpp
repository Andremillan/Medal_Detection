...
//	p_BMP_INFO->biSizeImage		= 0;				// The image size.
	...
	// 2011-12-03 Kim Song Gun
	p_BMP_INFO->biSizeImage		= LM_SIZEOF_4(p_X) * p_Y;		// The image size.
...
//	p_BMP_INFO->biClrUsed		= 0;				// The number of color indexes in the color table.
	...
	// 2011-12-03 Kim Song Gun
	p_BMP_INFO->biClrUsed		= 0x100;			// The number of color indexes in the color table.
...
,	int					p_inf				//:IN :8=grayA24=color
	...
	// 2011-12-03 Kim Song Gun // Add defaultvalue
	w_sts = ImgToBmp(w_x,w_y,w_inf,w_pBuff,(char*)p_buff , 0) ;
	if ( w_sts != 0 ) {
		w_sts = -11	;
		goto L_EXIT ;
	}
